...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        # here we just train it 10x times in different ways
        for validation_step in range(1,11):
            ...
            train_data_gen = utils.CrossValidationBatchGenDeepSupervision(train_data_loader, discretizer,
                                                            normalizer, args.batch_size, return_names=True, 
                                                            validation_mode = 2 , validation_step = validation_step)
            val_data_gen = utils.CrossValidationBatchGenDeepSupervision(val_data_loader, discretizer,
                                                        normalizer, args.batch_size, return_names=True, 
                                                        validation_mode = 0 , validation_step = validation_step)
            ...
            model = AdaCare(args.rnn_dim, args.kernel_size, args.kernel_num, args.input_dim, args.output_dim, 
                            args.dropout_rate, args.r_visit, args.r_conv, args.activation_func, device).to(device)
            optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
